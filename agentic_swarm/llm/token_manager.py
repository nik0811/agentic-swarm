...
class TokenManager:
    ...
    def _get_encoding(self):
        if self._encoding is None:
            try:
                ...
                raise ImportError(
                    "tiktoken package not installed. Run: pip install tiktoken"
                ) from None
    ...
    def calculate_budget(
        ...
        cfg = get_config().llm
        reserved_output = (
            reserved_output if reserved_output is not None else cfg.reserved_output_tokens
        )
        system_prompt_tokens = (
            system_prompt_tokens if system_prompt_tokens is not None else cfg.system_prompt_tokens
        )
