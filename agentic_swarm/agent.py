...
from .tools.discovery import ToolRegistry as ToolDiscovery
from .tools.discovery import ToolSelector
from .tools.discovery import get_global_registry as _get_discovery_registry
...
def register_tool(tool: Tool, category: str = "general", keywords: list[str] = None) -> Tool:
...
class Agent:
    ...
    def __init__(
        ...
        if auto_tools:
            ...
            if isinstance(auto_tools, str):
                ...
                category_filter = (
                    tool_categories[0] if tool_categories and len(tool_categories) > 0 else None
                )
    ...
    async def remember(self, content: str, metadata: dict = None) -> str | None:
        """
        ...
        try:
            ...
            if self._tool_selector and self._tool_retry > 0:
                result = await self._tool_selector.execute_with_retry(
                    task_description=tool.description, arguments=arguments
                )
                if result.success:
                    return result.result
                raise ToolNotFoundError(
                    f"Tool '{name}' and alternatives failed: {result.error}"
                ) from e
            raise
    ...
    async def create_agent(
        self, name: str, role: str, tools: list[Tool] = None, **kwargs
    ) -> "Agent":
        ...
        child = await self._spawner.spawn(
            parent=self, name=name, role=role, tools=tools, spawner=self._spawner, **kwargs
        )
        return child
    ...
    async def send(self, target: "Agent", message: str) -> None:
        ...
        target._recall_memory.push(f"Message from {self.name}: {message}", role="system")
