...
from ..lifecycle.healer import Healer
from ..lifecycle.supervisor import AgentHealth, Supervisor
...
class IIIWorkerBridge:
    ...
    def __init__(
        ...
        if not III_AVAILABLE:
            raise ImportError("iii-sdk is not installed. Install with: pip install iii-sdk")
    ...
    async def _on_agent_failure(self, agent: Agent, health: AgentHealth):
        ...
        print(
            f"[IMMORTAL] Attempting recovery ({self._recovery_count + 1}/{self._max_recovery_retries})..."
        )

        try:
            # Attempt recovery with healer
            recovered = await self._healer.recover(
                agent, Exception(health.last_error or "Unknown error")
            )
