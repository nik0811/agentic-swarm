...
class Spawner:
    ...
    def can_spawn(self, parent_id: str) -> bool:
        ...
        return not len(children) >= self._max_children

    async def spawn(
        self, parent: Agent, name: str, role: str, tools: list = None, **kwargs
    ) -> Agent:
        ...
        if not self.can_spawn(parent.id):
            raise AgentCreationError(
                f"Cannot spawn more children for agent {parent.id}: max_depth={self._max_depth}, max_children={self._max_children}"
            )

        child = Agent(
            name=name, role=role, tools=tools, llm_router=parent.llm_router, parent=parent, **kwargs
        )
    ...
    def remove_agent(self, agent_id: str) -> None:
        ...
        for _parent_id, children in self._agent_tree.items():
